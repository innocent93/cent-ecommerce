...
export const errorHandler = (err, req, res, next) => {
  ...
  // Only genuine server errors page anyone / count against error budgets —
  // a bad login attempt or a validation failure is expected traffic, not
  // an incident.
  if (isServerError) {
    captureException(err, { statusCode, path: req.originalUrl, method: req.method, requestId: req.id });
  }
