...
import { cached, invalidateByPrefix } from './cache.service.js';
...
export const createProduct = async (body, files, sellerId = null) => {
...
export const listProducts = async (query) => {
  ...
  // Cache key captures every input that changes the output — two requests
  // with different filters/pages/currency never collide. NOTE: this caches
  // browsing reads only. Checkout's stock reservation (order.service.js)
  // always reads Product directly, uncached — a product can never be
  // oversold because of a stale cache entry here, only *displayed* as
  // available for up to CACHE_TTL_SECONDS after it actually sells out,
  // which is the standard, accepted trade-off e-commerce sites make for
  // read-scaling a product catalog.
  const cacheKey = `${PRODUCT_CACHE_PREFIX}list:${JSON.stringify({ page, limit, category, subCategory, bestseller, brand, gender, search, currency })}`;
...
export const removeProduct = async (id, ownerSellerId = null) => {
...
export const updateProduct = async (productId, body, files = {}, ownerSellerId = null) => {
...
export const getProductById = async (productId, currency) => {
  const cacheKey = `${PRODUCT_CACHE_PREFIX}single:${productId}:${currency || 'base'}`;
  return cached(cacheKey, 60, async () => {
    const product = await Product.findById(productId);
    if (!product) throw ApiError.notFound('Product not found');
