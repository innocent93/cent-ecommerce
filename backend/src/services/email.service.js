...
// Every send goes through here. Deliberately swallows errors (logs them,
// doesn't throw) — a failed email should never fail the checkout/signup/etc.
// request that triggered it. If SMTP isn't configured at all, this just
// logs what *would* have been sent, which is handy in local development.
const send = async ({ to, subject, html }) => {
  const t = getTransporter();
  if (!t) {
    logger.info({ to, subject }, 'Email not sent (SMTP not configured) — would have sent');
    return { sent: false };
  }
  try {
