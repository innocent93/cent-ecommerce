...
// POST /api/user/google — "Continue with Google". Body: { idToken } (the
// credential returned by Google Identity Services on the frontend).
export const googleLogin = asyncHandler(async (req, res) => {
  const { user, accessToken, refreshToken, isNewUser } = await userService.googleLogin(
    req.body.idToken,
    requestMeta(req)
  );
  setRefreshCookie(res, refreshToken);
  req.log.info({ userId: user._id.toString(), isNewUser }, 'User logged in via Google');
  return sendSuccess(res, {
