...
export const placeOrder = asyncHandler(async (req, res) => {
  ...
  const { orders, paystackAuthorizationUrl, isReplay } = await orderService.placeOrder(
    req.user.id,
  ...
  return sendSuccess(res, {
    ...
    // `order` (singular) kept for backward compatibility with clients built
    // against Option A, where a checkout always produced exactly one order
    // — it's the first (or only) resulting order. `orders` (plural) is the
    // complete, correct picture for a cart that split across sellers.
    order: orders[0],
    orders,
...
export const getMySellerOrders = asyncHandler(async (req, res) => {
  const { orders, pagination } = await orderService.getSellerOrders(req.seller.id, req.query);
  return sendSuccess(res, { orders, pagination });
});

export const updateMySellerOrderStatus = asyncHandler(async (req, res) => {
  const order = await orderService.updateSellerOrderStatus(req.seller.id, req.params.orderId, req.body, req.log);
  return sendSuccess(res, { message: 'Order updated', order });
});
