...
export const handlePaystackWebhook = asyncHandler(async (req, res) => {
  ...
  switch (event.event) {
    case 'charge.success': {
      order = await orderService.markOrderPaid(event.data.reference, req.log);
      break;
    }
    case 'charge.failed': {
      order = await orderService.markOrderPaymentFailed(event.data.reference, req.log);
      break;
    }
    default:
  ...
  // so this record survives even if order processing itself had a bug.
  await PaymentEvent.create({
