...
const orderSchema = new mongoose.Schema(
  {
    ...
    // Marketplace: unset means this order is for platform-owned products
    // (Option A behavior). Set means every item in this order belongs to
    // one specific seller — a cart spanning multiple sellers is split into
    // one Order per seller at checkout time (see order.service.js#placeOrder),
    // sharing a `checkoutSessionId` so they can be shown together as "one
    // purchase" in the customer's order history despite being separate
    // Order documents. This keeps every existing single-order code path
    // (tracking, refunds, status updates) working completely unchanged —
    // it operates on one seller's order at a time either way.
    seller: { type: mongoose.Schema.Types.ObjectId, ref: 'Seller', default: null, index: true },
    checkoutSessionId: { type: String, index: true },
    // Set once this order's commission-adjusted total has actually been
    // paid out to the seller — prevents the same order being included in
    // two separate payout runs (the most common real bug in marketplace
    // payout systems, per MARKETPLACE_MIGRATION.md).
...
// Covers payout.service.js#getEligibleOrdersFilter — { seller, paymentStatus:
// 'paid', status: 'delivered', payout: null } — run on every seller
// dashboard balance check and every payout run. Without this, Mongo falls
// back to the single-field `seller` index and filters the rest in memory,
// which is fine for a seller with dozens of orders but degrades linearly
// as any one seller's order history grows into the thousands.
orderSchema.index({ seller: 1, paymentStatus: 1, status: 1, payout: 1 });

// Covers order.service.js#getSellerOrders — { seller, status? } sorted by
// createdAt desc. Lets Mongo satisfy the sort from the index itself
// (no in-memory sort stage) for a seller's paginated order list, the most
// frequently hit query in the seller dashboard.
